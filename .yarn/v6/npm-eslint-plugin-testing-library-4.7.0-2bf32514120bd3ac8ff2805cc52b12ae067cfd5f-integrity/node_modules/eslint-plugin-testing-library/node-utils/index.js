...
function getStatementCallExpression(statement) {
    if (is_node_of_type_1.isExpressionStatement(statement)) {
        const { expression } = statement;
        if (is_node_of_type_1.isCallExpression(expression)) {
            return expression;
        }
        if (experimental_utils_1.ASTUtils.isAwaitExpression(expression) &&
            is_node_of_type_1.isCallExpression(expression.argument)) {
            return expression.argument;
        }
        if (is_node_of_type_1.isAssignmentExpression(expression)) {
            if (is_node_of_type_1.isCallExpression(expression.right)) {
                return expression.right;
            }
            if (experimental_utils_1.ASTUtils.isAwaitExpression(expression.right) &&
                is_node_of_type_1.isCallExpression(expression.right.argument)) {
                return expression.right.argument;
            }
        }
    }
    if (is_node_of_type_1.isReturnStatement(statement) && is_node_of_type_1.isCallExpression(statement.argument)) {
        return statement.argument;
    }
    if (is_node_of_type_1.isVariableDeclaration(statement)) {
        for (const declaration of statement.declarations) {
            if (is_node_of_type_1.isCallExpression(declaration.init)) {
...
function isEmptyFunction(node) {
    if (experimental_utils_1.ASTUtils.isFunction(node) && is_node_of_type_1.isBlockStatement(node.body)) {
...
function findImportSpecifier(specifierName, node) {
    if (is_node_of_type_1.isImportDeclaration(node)) {
        const namedExport = node.specifiers.find((n) => {
            return (is_node_of_type_1.isImportSpecifier(n) &&
                [n.imported.name, n.local.name].includes(specifierName));
        });
        if (namedExport) {
            return namedExport;
        }
        return node.specifiers.find((n) => is_node_of_type_1.isImportNamespaceSpecifier(n));
    }
    else {
        ...
        if (!is_node_of_type_1.isObjectPattern(requireNode.id)) {
            return undefined;
        }
        const property = requireNode.id.properties.find((n) => is_node_of_type_1.isProperty(n) &&
